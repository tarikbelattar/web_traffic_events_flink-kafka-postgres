...
sessionized_job:
	docker compose exec jobmanager ./bin/flink run -py /opt/src/job/homework_sessionized_ip.py --pyFiles /opt/src -d
avg_session_job:
	docker compose exec jobmanager ./bin/flink run -py /opt/src/job/homework_aggregation_answers_job.py --pyFiles /opt/src -d
.PHONY: stop
